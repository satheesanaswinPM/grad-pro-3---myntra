from __future__ import annotations

from typing import Any

from src.ingest.adapters.base import CanonicalFeedback, build_record, cell, join_text, path_lcontains


class SocialAdapter:
    name = "social"

    def matches(self, relative_path: str) -> bool:
        return path_lcontains(
            relative_path,
            "social",
            "instagram",
            "twitter",
            "facebook",
            "tiktok",
        )

    def convert(
        self,
        raw: dict[str, Any],
        *,
        relative_path: str,
        line_no: int,
        ingest_at: str,
    ) -> CanonicalFeedback | None:
        return build_record(
            source="social",
            text=join_text(cell(raw, "text", "caption", "content", "body"), cell(raw, "title")),
            relative_path=relative_path,
            line_no=line_no,
            ingest_at=ingest_at,
            native_id=cell(raw, "id", "post_id"),
            source_url=cell(raw, "url", "permalink", "link"),
            authored_at=cell(raw, "created_at", "date", "timestamp"),
            user_key=cell(raw, "username", "user", "author"),
            raw=raw,
            extra_meta={"adapter": self.name},
        )
